...
/**
 * CSV Upload API for Bulk Unit Creation
 * POST /api/csv-upload/units
 */
export async function POST(request) {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Optional field: occupancy_status (validate if provided)
      if (row.occupancy_status && row.occupancy_status.trim() !== "") {
        const validStatuses = ["vacant", "occupied", "maintenance"];
        if (
          !validStatuses.includes(row.occupancy_status.trim().toLowerCase())
        ) {
          rowErrors.push({
            row: rowNumber,
            ...
            error:
              "Occupancy status must be one of: vacant, occupied, maintenance",
          });
        }
      }

      // Optional field: bedrooms
      if (row.bedrooms && row.bedrooms.trim() !== "") {
        const bedrooms = parseInt(row.bedrooms);
        if (isNaN(bedrooms) || bedrooms < 0) {
          rowErrors.push({
      ...
      // Optional field: property_type (validate if provided)
      if (row.property_type && row.property_type.trim() !== "") {
        ...
        if (!validTypes.includes(row.property_type.trim().toLowerCase())) {
          rowErrors.push({
            row: rowNumber,
            field: "property_type",
            value: row.property_type,
            error:
              "Property type must be one of: apartment, studio, bedsitter, house, commercial, office, shop",
          });
        }
      }
    ...
    for (const buildingId of buildingIds) {
      if (!existingBuildingIds.has(buildingId)) {
        ...
        dbErrors.push({
          ...
          field: "building_id",
          value: buildingId,
          error: `Building ID ${buildingId} does not exist in the database`,
        });
      }
    }
      ...
      return Response.json(
        {
          success: false,
          message:
            "Database validation failed. Some buildings do not exist or unit names are duplicates.",
          totalRows: rows.length,
          validRows: 0,
    ...
    // Third pass: Create units with transaction
