...
export default function TenantAssignmentPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch ALL units (not just unoccupied ones)
        ...
        // Fetch ALL tenants (not just those without active tenancies)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("/api/tenant-assignment", {
        method: "POST",
        ...
        <section className="content">
          <div className="container-xxl flex-grow-1 container-p-y">
            <div className="alert alert-warning">
              ...
              <p>
                There are no vacant units available for tenant assignment. All
                units are currently occupied.
              </p>
              <a href="/dashboard/property_records" className="btn btn-primary">
                View All Units
              </a>
            </div>
          </div>
        ...
        <div className="container-xxl flex-grow-1 container-p-y">
          ...
          <div className="tab-content mt-3">
                      ...
                      {form.tenant_type === "existing" && (
                        <div className="form-group mb-3">
                          ...
                          <small className="text-muted">
                            Note: You can assign any tenant, including those
                            with existing tenancies (many-to-many relationship).
                          </small>
                        </div>
                      )}
            ...
            {activeTab === "bulk" && (
              <div className="tab-pane fade show active">
                <CSVBulkUpload
                  uploadEndpoint="/api/csv-upload/tenant-assignments"
                  templateEndpoint="/api/csv-template?type=tenant-assignments"
                  title="Bulk Tenant Assignment Upload"
                  description="Upload a CSV file to assign multiple tenants to units at once. The CSV must include tenant_id, unit_id, and start_date columns."
                />
              </div>
            )}
          </div>
