...
/**
 * CSV Upload API for Bulk Tenant Creation and Assignments (New Tenants)
 * POST /api/csv-upload/tenant-assignments
 */
export async function POST(request) {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Optional field: deposit
      if (row.deposit && row.deposit.trim() !== "") {
        const deposit = parseFloat(row.deposit);
    ...
    // Second pass: Check database existence for units only
    const dbErrors = [];
    ...
    // Check if all units exist and get unit details including building info
    const existingUnits = await sql`
      SELECT u.id, u.name, u.rent_amount_kes, u.deposit_amount_kes, u.building_id, b.name as building_name 
      FROM units u
      LEFT JOIN buildings b ON u.building_id = b.id
      WHERE u.name = ANY(${unitNames})
    `;
    ...
    for (const row of validRows) {
      ...
      if (!matchingUnits || matchingUnits.length === 0) {
        dbErrors.push({
          ...
          field: "unit_name",
          value: row.unit_name,
          error: `Unit "${row.unit_name}" does not exist in database`,
        });
    ...
    if (dbErrors.length > 0) {
      const response = {
        success: false,
        message:
          "Database validation failed. Some phone numbers already exist, unit names do not exist, or unit names are ambiguous.",
        totalRows: rows.length,
        validRows: 0,
    ...
    try {
      for (const row of validRows) {
        try {
          ...
          const finalDeposit =
            row.deposit !== null ? row.deposit : unit.deposit_amount_kes;
